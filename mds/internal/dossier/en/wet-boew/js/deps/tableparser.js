    ...
    var componentName = "wb-tableparser",
        ...
        init = function (event) {
            ...
            function processRowgroupHeader(colgroupHeaderColEnd) {
                ...
                for (i = 0, iLen = theadRowStack.length; i !== iLen; i += 1) {
                    ...
                    for (j = 0, jLen = theadRS.cell.length; j < jLen; j += 1) {
                        ...
                        if (!cell.descCell &&
                            cell.elem.nodeName.toLowerCase() === "th" && !cell.type &&
                            theadRSNext &&
                            theadRSNext.uid !== cell.uid &&
                            theadRSNextCell && !theadRSNextCell.type &&
            ...
            function rowgroupSetup(forceDataGroup) {
                ...
                if (rowgroupHeaderRowStack.length === 0 && lstRowGroup.length > 0 && !currentRowGroup.type && colgroupFrame[0] &&
                    ( colgroupFrame[0].type === 1 || ( !colgroupFrame[0].type && colgroupFrame.length > 0 ) ) && !forceDataGroup) {
                ...
                for (i = 0; i < headStackLength; i += 1) {
                    ...
                    for (j = 0, cellLength = currRow.cell.length; j < cellLength; j += 1) {
                        ...
                        if (( currCell.type === 1 || currCell.type === 7 ) && (
                                !( j > 0 && currCell.uid === currRow.cell[j - 1].uid ) && !( i > 0 && currCell.uid === tblparser.theadRowStack[i - 1].cell[j].uid )
                            )) {
